...
- **任意路由即可**：不再需要停在「测验与作业」页面，公告页、课件页都能触发全课程刷新（打开页面即刷新，30 分钟内不重复；另有 12 小时周期兜底）
...
- ⚠️ **首次仍需载入课程页面** — 插件需要已登录的同源页面和已保存课程路由。首次打开任意 MOOC/SPOC 学习页后，后续刷新可自动创建临时非激活代理页；登录失效或页面无法加载时会超时失败
