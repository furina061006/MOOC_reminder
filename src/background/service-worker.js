...
const MESSAGE_HANDLERS = {
  ...
  async TEST_NOTIFICATION(msg) {
    ...
    const digestItems = [
      '第一章作业', '第二章测验', '第三章作业', '期中考试', '第四章作业'
    ].map(function(title, index) {
      return {
        title,
        courseName: index === 3 ? '大学英语' : '数据结构',
        deadline: new Date(digestNow.getTime() + (index + 1) * 3 * 60 * 60 * 1000).toISOString(),
        checkedOff: false
      };
    });
    const copy = {
      ...
      digest: { title: '今日 MOOC 作业汇总', message: formatDigestMessage(digestItems, digestNow), priority: 1 }
    }[kind];
    try {
