注: 待追踪任务
  - **2026-09-20 新观察（用户实测，两条独立现象）**：
    (a) **SPOC 作业「开窗口即可抓」，普通 MOOC 作业「必须位于当前页面才能抓」**。
        **根因已确认并修复（2026-09-21 第二轮）**：长期挂后台的标签页会被浏览器冻结、
        甚至被 Memory Saver 卸载，这类页面没有可用的内容脚本——既不能上报课程，也不能接手
        `BATCH_API_FETCH`。旧流程按 `lastAccessed` 挑一个页面就发批量，挑中冻结页会白等 90 秒
        超时并把整轮判失败（这就是「间歇」的来源），而且课程列表非空时再也不会去问那些页面
        （这就是「那门课一直缺」的来源）。现在每轮抓取都先问所有已打开页面（1.5s 上限）：
        能应答的页面顺便重新登记课程，批量只发给刚应答过的学习页，一个都没有就用临时代理页。
        用户 2026-09-21 复现于「复变函数与积分变换」，并自述「过一会儿又好了」——与超时失败后
        下一轮换到活页面才成功一致。细节见 `.claude/logs/2026-09-21-clear-data-course-recovery.md`。
        **待浏览器验证**。
