...
## 第四轮（同日）：日志规则不再按「目录名」一刀切

用户提出：「log 是可以上传的，这是开源项目，日志应该共享？」

事实核对：**项目日志本来就全部共享着**——`.dsh/logs/` 下 20 个文件 / 180KB（changelog + 19 篇日期化
开发日志）全都在 git 里。被忽略的只有「运行时输出」这一类。

但这条追问暴露了 `.gitignore` 里一个真正的设计缺陷：原来的规则是

```gitignore
logs/     # 忽略任何层级的、叫 logs 的目录
```

而本仓库里唯一叫 `logs` 的目录恰恰是**要共享**的 `.dsh/logs/`。这条规则保护不了任何东西
（仓库里没有任何运行时日志），却已经害我们丢过一整批开发日志——就是为了给它擦屁股，才需要
`!.dsh/logs/**` 这种否定规则，而否定规则一旦漏写，`git status` 还看不出异常。
...
- 运行时输出照样进不了仓库；
- **任何叫 `logs` 的目录都能正常跟踪**，那一类陷阱被彻底移除（不需要再靠否定规则救场）；
- `.dsh/*` + 两条否定（`logs/`、`agents/`）保持不变，仍然只放行这两个文档子目录。

测试同步加了反向守卫：`npm-debug.log` / `debug.log` 必须被忽略，而 `.dsh/logs/changelog.md`
与 `src/logs/notes.md`（任何叫 logs 的目录）必须**不**被忽略——把「不要按目录名一刀切」这条教训
固化成断言。134 个测试通过。
