...
> **验证状态**：根因有**真实页面 DOM**（另存的 `/home.htm#/home/spocCourse`）与单测覆盖，
> 新测试做了负向验证（去掉守卫立刻失败）；但**修复本身尚未在浏览器里逐项确认** ——
> 重载扩展 → 删除已存在的幽灵课程 → 再刷新那个 SPOC 课程列表页，即可确认它不再出现。

- **修复「没选过的课莫名出现在已追踪课程里」**：首页右侧「最近发表」里的论坛帖链接（形如 `/learn/{课程}?tid=…`）会被当成课程登记——帖子的标题就成了课程名，于是出现「课名是章节/帖子名」「实际是 SPOC 却显示普通」「0 条作业」这类幽灵课程（例如 `NEU-1474956162 · 普通 · 牛顿第二定律`）。现在**指向课程内部某条内容的链接一律不再当作课程**（课程页上的源课程/章节链接同样不再采集），并且**来自页面链接的名字不能覆盖已有的课程名**。已经产生的幽灵课程请在设置页「已追踪课程」里删除。
- **课程名不会被「空名字」清空**：课程页一时读不到标题时，之前会拿空名字覆盖已存的课程名（列表里变成「未知课程」）。现在空名字一律不写。

## 1.1.1（2026-09-22）

> **验证状态**：本版两条修复都有代码与单测覆盖（含负向验证：把旧写法改回去，新测试立刻失败），
> 尚未在浏览器里逐项确认。

- **修复「更新到新版本后，设置页仍显示旧版本号、还提示有新版本」**：设置页里的「当前版本」显示的是**上一次检查时**存下的快照，而重新加载扩展并不会触发一次新检查（后台只在每 12 小时、或你手点「检查更新」时才检查）。于是刚装好 1.1.0 的用户会看到「当前版本 v1.0.0 / 发现新版本 v1.1.0」。现在页面会先把这份快照对齐到**正在运行的版本**再显示：已经装上的版本不再被提示为「有新版本」，「最新版本」与「上次检查时间」照旧显示。
- **修复「本次检查失败」的提示被吞掉**：当上次的结论是「有新版本」而这一次查询失败时，页面只显示那句旧的「发现新版本」，完全不提这次没查成——于是呈现出「当前版本 = 最新版本，却让你去下载」的自相矛盾状态，而且毫无解释。现在失败原因会照实显示，并与「有新版本」提示并存。

## 1.1.0（2026-09-21）

> **验证状态**：标 ✅ 的条目已在真实浏览器里确认生效（清除数据后刷新可恢复、
> 课程页挂后台也能抓到、作业跳转目标完全正确）。其余条目目前只有代码与单测覆盖，尚未在浏览器逐项验证。
> 「同一课程页面只抓取一次」是 30 分钟节流的设计行为，本版未改动，见 `backlog.md`。
...
- **新增「反馈入口」**：popup 页脚与设置页底部现在能直接打开 GitHub Issues（推荐走这里——别人能搜到同样的问题，模板会引导你附上版本与日志，通常一轮就能定位）；没有 GitHub 账号、或涉及账号信息不想公开时，同一处仍保留维护者邮箱。
